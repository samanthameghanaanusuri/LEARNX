...
def sync_python_course():
    ...
    logger.info("RUN_PYTHON_SYNC detected")
    
    driver = db.engine.url.drivername
    logger.info(f"SQLAlchemy database driver: {driver}")
    
    if driver == "sqlite":
        logger.error("CRITICAL ERROR: Connected to SQLite database. Refusing to run production sync. Halting operation.")
        raise RuntimeError("Production synchronization must not run against SQLite fallback.")
    ...
    total_courses_before = Course.query.count()
    logger.info(f"Total Courses BEFORE: {total_courses_before}")
    ...
    try:
        if not course:
            logger.info("Course 'python-programming' not found in database. Creating base record...")
            course = Course(
                title="Python Programming — Beginner to Advanced",
                slug="python-programming",
                description="Master Python from the basics to advanced concepts like OOP, Decorators, and Algorithms. Includes quizzes, exercises, and knowledge tracing.",
            ...
            db.session.commit()
            logger.info("Base course record created.")
        ...
        total_courses_after = Course.query.count()

        logger.info("\n=== POST-SYNCHRONIZATION METRICS ===")
        logger.info(f"Total Courses AFTER       : {total_courses_after}")
        logger.info(f"Python Course ID          : {course.id}")
        logger.info(f"Expected Module Count     : {expected_mod_count}")
        ...
        db.session.rollback()
        logger.error(f"CRITICAL ERROR: Synchronization failed with error: {e}")
        raise e

if __name__ == '__main__':
    # Only used for local direct testing; production triggers this via run.py
